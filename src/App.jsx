...
function SectionHeader({ title, action, onAction }) {
  return (
    <div className="flex items-center justify-between mb-3">
      <h2 style={{ fontFamily: "Fraunces, serif", fontWeight: 600, fontSize: 19, color: C.ink }}>{title}</h2>
      {action && (
        <button onClick={onAction} style={{ color: C.primary, fontWeight: 600, fontSize: 13.5 }} className="flex items-center gap-1">
...
function EmptyState({ text, cta, onCta }) {
  return (
    <div className="flex flex-col items-center justify-center text-center py-8 px-4" style={{ color: C.ink50 }}>
      ...
      {cta && (
        <button onClick={onCta} style={{ color: C.primary, fontWeight: 600, fontSize: 13.5, marginTop: 6 }}>
          {cta}
        </button>
      )}
    </div>
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 50 }} className="flex items-end justify-center">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 style={{ fontFamily: "Fraunces, serif", fontWeight: 600, fontSize: 18, color: C.ink }}>{title}</h3>
...
function TextField({ label, ...props }) {
  return (
    <label className="block mb-3">
      {label && (
        <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>{label}</span>
      )}
      <input
        {...props}
        style={{
          width: "100%", background: C.card, border: `1px solid ${C.line}`, borderRadius: 14,
          padding: "11px 14px", fontSize: 14.5, color: C.ink, fontFamily: "Inter, sans-serif",
        }}
      />
    </label>
  );
...
function PrimaryButton({ children, ...props }) {
  return (
    <button
      {...props}
      style={{
        width: "100%", background: C.primary, color: "#fff", fontWeight: 700, fontSize: 15,
...
function IdentityGate({ onPick }) {
  return (
    <div className="flex flex-col items-center justify-center h-full px-8 text-center" style={{ background: C.paper }}>
      ...
      <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 30, color: C.ink }}>Homie</h1>
      <p style={{ color: C.ink70, fontSize: 14.5, marginTop: 8, marginBottom: 32, lineHeight: 1.5 }}>
        One shared space for shopping, the calendar, the pets, and who paid for what.
      </p>
      <p style={{ color: C.ink50, fontSize: 12.5, fontWeight: 600, marginBottom: 12 }}>WHO'S OPENING THE APP?</p>
      <div className="flex gap-3 w-full">
        {USERS.map((u) => (
          <button
            key={u} onClick={() => onPick(u)}
            style={{ flex: 1, padding: "16px 0", borderRadius: 18, background: C.card, border: `1.5px solid ${C.line}`, fontWeight: 700, fontSize: 15.5, color: C.ink }}
          >
            {u}
          </button>
        ))}
      ...
      {items.map(({ key, label, Icon, badge }) => {
        ...
        return (
          <button key={key} onClick={() => setTab(key)} className="flex-1 flex flex-col items-center gap-1" style={{ position: "relative", padding: "4px 0" }}>
            ...
            {!!badge && (
              <span style={{ position: "absolute", top: -2, right: "26%", background: C.primary, color: "#fff", fontSize: 9, fontWeight: 700, borderRadius: 999, minWidth: 15, height: 15, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 3px" }}>
                {badge}
              </span>
            )}
            <span style={{ fontSize: 10.5, fontWeight: 600, color: active ? C.primary : C.ink30 }}>{label}</span>
          </button>
        );
      })}
    </div>
...
function HomeTab({ identity, shopping, events, pets, budgets, expenses, setTab }) {
  ...
  return (
    <div className="px-5 pt-6 pb-4">
      <p style={{ color: C.ink50, fontSize: 13, fontWeight: 600 }}>Welcome back</p>
      <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 26, color: C.ink, marginBottom: 20 }}>{identity}</h1>
      <div className="grid grid-cols-4 gap-2 mb-7">
        {stats.map((s) => (
          <button key={s.label} onClick={() => setTab(s.tab)} style={{ background: s.soft, borderRadius: 18, padding: "12px 6px" }} className="flex flex-col items-center">
            <span style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 19, color: s.color }}>{s.value}</span>
            <span style={{ fontSize: 10.5, color: C.ink70, fontWeight: 600, marginTop: 2, textAlign: "center" }}>{s.label}</span>
          </button>
        ))}
      </div>
          ...
          upcomingEvents.map((e, i) => (
            <div key={e.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
              <div style={{ width: 42, textAlign: "center", background: e.type === "shared" ? C.primarySoft : C.mintSoft, borderRadius: 12, padding: "5px 0" }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: e.type === "shared" ? C.primary : C.mint }}>{new Date(e.date + "T00:00:00").getDate()}</div>
              </div>
              <div className="flex-1">
                <p style={{ fontWeight: 600, fontSize: 14, color: C.ink }}>{e.title}</p>
                <p style={{ fontSize: 12, color: C.ink50 }}>{e.owner} · {e.type}</p>
              </div>
            </div>
          ))
        )}
        ...
        {openShopping.length === 0 ? (
          ...
          openShopping.slice(0, 4).map((item, i) => (
            <div key={item.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
              ...
              <span style={{ fontSize: 14, color: C.ink, flex: 1 }}>{item.text}</span>
              <span style={{ fontSize: 11, color: C.ink50, background: C.paper, padding: "3px 9px", borderRadius: 999 }}>{item.category}</span>
            </div>
          ))
        )}
      </div>
        ...
        {petTasks.length === 0 ? (
          ...
          petTasks.slice(0, 3).map((t) => (
            <div key={t.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: `1px solid ${C.line}` }}>
              ...
              <div className="flex-1">
                <p style={{ fontWeight: 600, fontSize: 14, color: C.ink }}>{t.text}</p>
                <p style={{ fontSize: 12, color: C.ink50 }}>{t.petName}</p>
              </div>
            </div>
          ))
        )}
  ...
  return (
    <div className="px-5 pt-6 pb-4">
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 22, color: C.ink }}>Shopping list</h1>
      </div>
      <p style={{ color: C.ink50, fontSize: 12.5, marginBottom: 16 }}>Shared by {USERS.join(" & ")}</p>
      <div className="flex gap-2 mb-4 overflow-x-auto">
        {["All", ...SHOP_CATS.map((c) => c.key)].map((cat) => {
          const active = filter === cat;
          return (
            <button key={cat} onClick={() => setFilter(cat)} style={{ padding: "7px 14px", borderRadius: 999, fontSize: 12.5, fontWeight: 600, whiteSpace: "nowrap", background: active ? C.primary : C.card, border: `1px solid ${active ? C.primary : C.line}`, color: active ? "#fff" : C.ink70 }}>
              {cat}
            </button>
          );
        })}
          ...
          items.map((item, i) => {
            ...
            return (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
                ...
                <span style={{ fontSize: 14, flex: 1, color: item.done ? C.ink30 : C.ink, textDecoration: item.done ? "line-through" : "none" }}>{item.text}</span>
                <span style={{ fontSize: 11, color: catInfo?.color || C.ink50, background: catInfo?.soft || C.paper, padding: "3px 9px", borderRadius: 999, fontWeight: 600 }}>{item.category}</span>
      ...
      <div style={{ background: C.card, borderRadius: 18, border: `1px solid ${C.line}`, padding: 12 }}>
        <div className="flex gap-2 mb-2 overflow-x-auto">
          {SHOP_CATS.map((c) => {
            ...
            return (
              <button key={c.key} onClick={() => setDraftCat(c.key)} style={{ padding: "5px 11px", borderRadius: 999, fontSize: 11.5, fontWeight: 600, whiteSpace: "nowrap", background: active ? c.color : c.soft, color: active ? "#fff" : c.color }}>
                {c.key}
              </button>
            );
          })}
        ...
        <div className="flex gap-2">
          <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} placeholder="Add an item..." style={{ flex: 1, border: "none", outline: "none", fontSize: 14.5, color: C.ink, background: "transparent" }} />
    ...
    <div className="px-5 pt-6 pb-4">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2.5">
          ...
          <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 22, color: C.ink }}>Calendar</h1>
        </div>
        <div className="flex" style={{ background: C.paper, borderRadius: 999, padding: 3 }}>
          {["shared", "personal"].map((s) => (
            <button key={s} onClick={() => setScope(s)} style={{ padding: "5px 12px", borderRadius: 999, fontSize: 11.5, fontWeight: 700, textTransform: "capitalize", background: scope === s ? C.card : "transparent", color: scope === s ? C.ink : C.ink50 }}>{s}</button>
          ))}
        </div>
      </div>
      <div className="flex gap-2 my-4">
        {[{ k: "month", l: "Month" }, { k: "mood", l: "Mood" }].map((t) => (
          <button key={t.k} onClick={() => setView(t.k)} style={{ padding: "7px 16px", borderRadius: 999, fontSize: 13, fontWeight: 600, background: view === t.k ? C.primary : C.card, border: `1px solid ${view === t.k ? C.primary : C.line}`, color: view === t.k ? "#fff" : C.ink70 }}>{t.l}</button>
        ))}
      </div>
      ...
      {view === "month" && (
        <>
          <div className="flex items-center justify-between mb-3">
            ...
            <span style={{ fontFamily: "Fraunces, serif", fontWeight: 600, fontSize: 16, color: C.ink }}>{MONTH_NAMES[cursor.m]} {cursor.y}</span>
          ...
          <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, padding: 14, marginBottom: 16 }}>
            <div className="grid grid-cols-7 mb-2">
              {DOW.map((d, i) => <div key={i} style={{ textAlign: "center", fontSize: 11, color: C.ink30, fontWeight: 700 }}>{d}</div>)}
            </div>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button key={day} onClick={() => setSelected(iso)} className="flex flex-col items-center" style={{ padding: "5px 0" }}>
                    <div style={{ width: 30, height: 30, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: isToday ? 700 : 500, background: isSel ? C.primary : "transparent", border: isToday && !isSel ? `1.5px solid ${C.primary}` : "none", color: isSel ? "#fff" : C.ink }}>{day}</div>
          ...
          <div className="flex items-center justify-between mb-2">
            <span style={{ fontWeight: 600, fontSize: 14, color: C.ink }}>{fmtDateLabel(selected)}</span>
            ...
            {dayEvents.length === 0 ? (
              ...
              dayEvents.map((e, i) => (
                <div key={e.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
                  ...
                  <div className="flex-1">
                    <p style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>{e.title}</p>
                    <p style={{ fontSize: 11.5, color: C.ink50 }}>{e.owner} · {e.type}</p>
                  </div>
      ...
      {view === "mood" && (
        <>
          <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, padding: 16, marginBottom: 18 }}>
            <p style={{ fontWeight: 600, fontSize: 14.5, color: C.ink, marginBottom: 12 }}>How are you feeling today?</p>
              ...
              {MOODS.map((m) => {
                ...
                return (
                  <button key={m.key} onClick={() => setTodayMood(m.key)} className="flex flex-col items-center gap-1.5">
                    <div style={{ width: 48, height: 48, borderRadius: 999, fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center", background: active ? m.color : C.paper, border: active ? `2px solid ${m.color}` : `1px solid ${C.line}` }}>{m.emoji}</div>
                    <span style={{ fontSize: 10.5, fontWeight: 600, color: active ? C.ink : C.ink50 }}>{m.key}</span>
                  </button>
                );
              })}
            </div>
              ...
              recentMoods.map((m, i) => {
                ...
                return (
                  <div key={m.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
                    <div style={{ width: 32, height: 32, borderRadius: 999, background: info ? `${info.color}22` : C.paper, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>{info?.emoji || "🙂"}</div>
                    <div className="flex-1">
                      <p style={{ fontSize: 14, color: C.ink }}><b>{m.owner}</b> felt <b style={{ color: C.primary }}>{m.mood.toLowerCase()}</b></p>
                      <p style={{ fontSize: 11.5, color: C.ink50 }}>{fmtDateLabel(m.date)}</p>
                    </div>
                  </div>
                );
              })
      ...
      {showAdd && (
        <Sheet title={`Add event · ${fmtDateLabel(selected)}`} onClose={() => setShowAdd(false)}>
          ...
          <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>Type</span>
    ...
    <div className="px-5 pt-6 pb-4">
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2.5">
          ...
          <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 22, color: C.ink }}>My pets</h1>
        </div>
      ...
      <p style={{ color: C.ink50, fontSize: 12.5, marginBottom: 16 }}>{pets.length} pet{pets.length === 1 ? "" : "s"} in your care</p>
      {pets.length === 0 ? (
        <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}` }}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span style={{ fontWeight: 700, fontSize: 16, color: C.ink }}>{p.name}</span>
                      <span style={{ fontSize: 11, color: C.ink50 }}>{p.gender}</span>
                    </div>
                    <p style={{ fontSize: 12.5, color: C.ink50 }}>{p.breed || p.species}</p>
                  </div>
                ...
                <div className="flex gap-2 mb-3">
                  {p.age && (
                    <div style={{ flex: 1, background: C.paper, borderRadius: 12, padding: "8px 10px" }}>
                      <p style={{ fontSize: 10, color: C.ink50, fontWeight: 600 }}>AGE</p>
                      <p style={{ fontSize: 13, fontWeight: 700, color: C.ink }}>{p.age}</p>
                    </div>
                  )}
                  {p.weight && (
                    <div style={{ flex: 1, background: C.paper, borderRadius: 12, padding: "8px 10px" }}>
                      <p style={{ fontSize: 10, color: C.ink50, fontWeight: 600 }}>WEIGHT</p>
                      <p style={{ fontSize: 13, fontWeight: 700, color: C.ink }}>{p.weight}</p>
                    </div>
                  )}
                </div>
                {openTasks.length > 0 && (
                  <div className="flex flex-col gap-1.5 mb-2">
                    {openTasks.map((t) => (
                      <button key={t.id} onClick={() => toggleTask(p.id, t.id)} className="flex items-center gap-2" style={{ background: C.coralSoft, borderRadius: 10, padding: "7px 10px" }}>
                        ...
                        <span style={{ fontSize: 12.5, color: C.ink, textAlign: "left" }}>{t.text}</span>
                      </button>
                    ))}
                  </div>
                )}
                <button onClick={() => setTaskFor(p.id)} style={{ fontSize: 12.5, fontWeight: 600, color: C.primary }}>+ Add task</button>
              </div>
            );
          })}
        </div>
      ...
      {showAdd && (
        <Sheet title="Add a pet" onClose={() => setShowAdd(false)}>
          ...
          <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>Species</span>
          ...
          <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>Gender</span>
  ...
  return (
    <div className="px-5 pt-6 pb-4">
      <div className="flex items-center gap-2.5 mb-1">
        ...
        <h1 style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 22, color: C.ink }}>Finance</h1>
      </div>
      <p style={{ color: C.ink50, fontSize: 12.5, marginBottom: 16 }}>{MONTH_NAMES[new Date().getMonth()]} {new Date().getFullYear()} · Shared expenses</p>
      <div style={{ background: `linear-gradient(135deg, ${C.primary}, ${C.primaryDark})`, borderRadius: 22, padding: 20, marginBottom: 20, color: "#fff" }}>
        <p style={{ fontSize: 11.5, opacity: 0.8, fontWeight: 700, letterSpacing: 0.3 }}>THIS MONTH'S SPENDING</p>
        <p style={{ fontFamily: "Fraunces, serif", fontWeight: 700, fontSize: 32, margin: "4px 0 16px" }}>{fmtBaht(totalSpend)}</p>
        <div className="flex gap-4 mb-4">
          {USERS.map((u) => (
            <div key={u} style={{ flex: 1 }}>
              <p style={{ fontSize: 11.5, opacity: 0.8 }}>{u} paid</p>
              <p style={{ fontWeight: 700, fontSize: 16 }}>{fmtBaht(paidByUser(u))}</p>
          ...
          {USERS.map((u) => (
            <div key={u} style={{ flex: 1, background: "rgba(255,255,255,0.16)", borderRadius: 14, padding: 12 }}>
              <div className="flex items-center gap-1.5 mb-1">
                ...
                <span style={{ fontSize: 11, opacity: 0.85 }}>{u} pending</span>
              </div>
              <p style={{ fontWeight: 700, fontSize: 15 }}>{fmtBaht(pendingByUser(u))}</p>
              <p style={{ fontSize: 10.5, opacity: 0.8 }}>{pendingCountByUser(u)} item{pendingCountByUser(u) === 1 ? "" : "s"} to settle</p>
            </div>
          ))}
        </div>
      </div>
      <div className="flex items-center justify-between mb-3">
        <span style={{ fontSize: 12, color: C.ink50, fontWeight: 700, letterSpacing: 0.3 }}>MONTHLY BUDGET</span>
        <button onClick={() => setShowAddBudget(true)} style={{ color: C.primary, fontWeight: 700, fontSize: 12.5 }}>+ Add category</button>
      </div>
            ...
            return (
              <div key={b.id} style={{ marginTop: i === 0 ? 0 : 16 }}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <span style={{ fontSize: 13, fontWeight: 700, color: over ? C.primary : C.ink70 }}>{fmtBaht(spent)} / {fmtBaht(b.limit)}</span>
      ...
      <div className="flex items-center justify-between mb-3">
        <span style={{ fontSize: 12, color: C.ink50, fontWeight: 700, letterSpacing: 0.3 }}>EXPENSES THIS MONTH</span>
      ...
      <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}` }}>
        {monthExpenses.length === 0 ? (
          ...
          monthExpenses.map((e, i) => (
            <div key={e.id} className="flex items-center gap-3 px-4 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
              <div className="flex-1">
                <p style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>{e.desc}</p>
                <p style={{ fontSize: 11.5, color: C.ink50 }}>{e.paidBy} · {e.category} · {fmtDateLabel(e.date)}</p>
              </div>
              <span style={{ fontWeight: 700, fontSize: 14, color: C.ink }}>{fmtBaht(e.amount)}</span>
      ...
      {showAddExpense && (
        <Sheet title="Add expense" onClose={() => setShowAddExpense(false)}>
          ...
          {budgets.length > 0 && (
            <>
              <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>Category</span>
              <PillSelect options={budgets.map((b) => b.name)} value={eForm.category} onChange={(v) => setEForm({ ...eForm, category: v })} />
            </>
          )}
          <span style={{ fontSize: 12.5, color: C.ink70, fontWeight: 600, display: "block", marginBottom: 5 }}>Paid by</span>
    ...
    <div className="px-5 pt-6 pb-4">
      ...
      <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, padding: 18, marginBottom: 20 }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <p style={{ fontWeight: 700, fontSize: 14.5, color: C.ink }}>Notifications</p>
        </div>
        <p style={{ fontSize: 12.5, color: C.ink50, marginBottom: 12 }}>
          {notifStatus === "granted"
            ? "On — you'll get a notification when the other person adds something while this app is open, and real push once Firebase Cloud Messaging is set up."
            : "Turn these on to hear when Putter or Q adds an item, event, or expense."}
        </p>
        {notifStatus !== "granted" && (
          <button onClick={onEnableNotifications} style={{ color: C.primary, fontWeight: 700, fontSize: 13.5 }}>
      ...
      <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, padding: 18, marginBottom: 20 }}>
        <div className="flex items-center gap-2 mb-2">
          ...
          <p style={{ fontWeight: 700, fontSize: 14.5, color: C.ink }}>About Homie</p>
        </div>
        <p style={{ fontSize: 13, color: C.ink70, lineHeight: 1.6 }}>
      ...
      <div style={{ background: C.card, borderRadius: 20, border: `1px solid ${C.line}`, padding: 18 }}>
        <p style={{ fontWeight: 700, fontSize: 14.5, color: C.primary, marginBottom: 6 }}>Reset all data</p>
        <p style={{ fontSize: 12.5, color: C.ink50, marginBottom: 12 }}>Clears shopping, calendar, pets and finance data for both of you. This can't be undone.</p>
        {!confirming ? (
          <button onClick={() => setConfirming(true)} style={{ color: C.primary, fontWeight: 700, fontSize: 13.5 }}>Reset data…</button>
        ) : (
          <div className="flex gap-2">
            <button onClick={() => { onResetAll(); setConfirming(false); }} style={{ flex: 1, background: C.primary, color: "#fff", fontWeight: 700, fontSize: 13, padding: "10px 0", borderRadius: 12 }}>Yes, reset</button>
            <button onClick={() => setConfirming(false)} style={{ flex: 1, background: C.paper, color: C.ink, fontWeight: 700, fontSize: 13, padding: "10px 0", borderRadius: 12 }}>Cancel</button>
          </div>
        )}
      </div>
    </div>
...
export default function App() {
  ...
  const enableNotifications = async () => {
    ...
    if (perm === "granted" && identity) {
      ...
      localNotify("Homie", "Notifications are on ✅");
    }
  };
